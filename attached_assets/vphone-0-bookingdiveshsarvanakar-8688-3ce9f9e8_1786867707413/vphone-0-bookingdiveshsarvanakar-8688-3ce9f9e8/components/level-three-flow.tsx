...
export function LevelThreeFlow({ taskKey, onComplete }: Props) {
  ...
  return <main className="mx-auto flex min-h-dvh max-w-6xl flex-col gap-6 px-4 pb-28 pt-8 text-foreground md:px-8 md:pb-8"><header className="flex items-center justify-between"><div><p className="text-sm font-bold uppercase tracking-widest text-primary">{t('level')} 3</p><h1 className="text-3xl font-black">{t('levelThreeSubtitle')}</h1></div><button onClick={() => window.history.back()} className="rounded-xl border border-border px-4 py-2 font-bold">{t('exitLabel')}</button></header><section className="grid gap-6 lg:grid-cols-[.7fr_1.3fr]"><aside className="rounded-3xl border border-border bg-card p-5"><p className="font-bold text-muted-foreground">{t('guidedTaskLabel')}</p><h2 className="mt-2 text-2xl font-black">{titles[Math.min(step,3)]}</h2><p className="mt-3 leading-6 text-muted-foreground">{t('takeActionNote')}</p><HintPanel taskKey={taskKey} hints={hints} /><div className="mt-6 flex flex-wrap gap-2">{titles.map((title,index)=><span key={title} className={`rounded-full px-3 py-1 text-xs font-bold ${index<=step?'bg-primary text-primary-foreground':'bg-secondary text-muted-foreground'}`}>{index+1}. {title}</span>)}</div></aside><div className="w-full max-w-sm flex-1"><VirtualPhone screenLabel={titles[Math.min(step,3)]}>{maps?<MapsScreen t={t} step={step} query={query} setQuery={setQuery} onNext={next} onFinish={finish}/>:payment?<PaymentScreen t={t} step={step} amount={amount} setAmount={setAmount} qr={qr} onNext={next} onFinish={finish}/>:<ReminderScreen t={t} step={step} label={label} setLabel={setLabel} daily={daily} setDaily={setDaily} onNext={next} onFinish={finish}/>}</VirtualPhone></div></section>{notice&&<p role="status" className="rounded-2xl bg-primary/10 p-4 text-center font-bold text-primary">{notice}</p>}<TaskControls onRepeat={repeatStep} onStartOver={restart} /></main>
}
