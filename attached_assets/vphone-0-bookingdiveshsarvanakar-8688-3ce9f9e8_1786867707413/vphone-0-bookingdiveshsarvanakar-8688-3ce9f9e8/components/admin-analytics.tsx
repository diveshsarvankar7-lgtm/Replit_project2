...
export function AdminAnalytics() {
  ...
  const detailedRows = useMemo(() => {
    ...
    return data.attempts.map((attempt, index) => {
      ...
      const participant = session ? participants.get(session.participant_id) : undefined
      const task = taskMap.get(attempt.task_key)
      return {
        record: index + 1,
        participantId: participant?.id ?? '',
        name: participant?.name ?? 'Local participant',
        age: participant?.age ?? '',
        sessionId: attempt.session_id,
        level: attempt.level_number,
        taskKey: attempt.task_key,
        task: task?.title ?? attempt.task_key,
        ...
        durationMinutes: duration(attempt.started_at, attempt.completed_at).toFixed(2),
      }
