...
export default function TaskPageContent() {
  ...
  const showTransition = searchParams.get('transition') === '1'
  ...
  const continueAfterComplete = () => {
    if (next?.task && next.levelNumber) {
      router.replace(`/task?level=${next.levelNumber}&task=${next.task.key}${next.levelComplete ? '&transition=1' : ''}`)
    } else {
      router.replace('/task/complete')
    }
  }

  if (showTransition && level && task) {
    return (
      <main className="flex min-h-dvh items-center justify-center px-6 py-12 text-center">
        <section className="w-full max-w-xl rounded-3xl border border-border bg-card p-8 shadow-sm">
          <p className="text-sm font-bold uppercase tracking-[0.18em] text-primary">Level {level.number - 1} complete</p>
          <h1 className="mt-3 text-4xl font-black text-balance">Getting ready for {level.title}</h1>
          <p className="mt-4 text-lg leading-relaxed text-muted-foreground">{level.subtitle}</p>
          <button type="button" onClick={() => router.replace(`/task?level=${level.number}&task=${task.key}`)} className="mt-8 rounded-2xl bg-primary px-6 py-4 font-bold text-primary-foreground">Continue to Level {level.number}</button>
        </section>
      </main>
    )
  }
